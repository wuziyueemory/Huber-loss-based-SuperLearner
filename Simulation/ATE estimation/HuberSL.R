...
# two stage super-learner with Square Loss and Huber Loss 
############################################### Square Loss #########################################################
...
############################## Change the Predict function in SuperLearner ####################################
# Only generate predictions for one-stage Huber loss-based super learner with partial-CV method
# Remove the option of onlySL

predict.onestage.huber_SL.partial_CV <- function(object, newdata, X = NULL, Y = NULL, ...) {
  ...
  k <- object$library.Num$stage.single
  predY <- matrix(NA, nrow = nrow(newdata), ncol = k)
  
  # Single-stage predictions
  for (mm in seq(k)) {
    family <- object$family$stage.single
    predY[, mm] <- do.call('predict', list(object = object$fitLibrary$stage.sinlge[[mm]],
                                           newdata = newdata,
                                           family = family,
                                           X = X,
                                           Y = Y))
  ...
  colnames(predY) <- object$libraryNames$`one-stage`
  ...
  # partial CV
  getPred <- 
    object$method$`Huber Loss`$computePred(predY = predY, 
                                           coef = object$coef$`one-stage`$`Huber-partial CV`, 
...
############################## Change the Predict function in SuperLearner ####################################
# Only generate predictions for one-stage Huber loss-based super learner with nested-CV method
# Remove the option of onlySL

predict.onestage.huber_SL.nested_CV <- function(object, newdata, X = NULL, Y = NULL, ...) {
  ...
  getPred <- 
    object$method$`Huber Loss`$computePred(predY = predY, 
                                           coef = object$coef$`one-stage`$`Huber-nested CV`, 
                                           control=object$control)
...
## Two-Stage Huber loss-based Super Learner
##############################################################################################################

HuberSL <- function(Y, X, newX = NULL, library.2stage, library.1stage, lambda,
